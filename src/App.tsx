...
function pickBalancedMatches(
  ...
  // Prioriza partidos con equipos menos jugados
  const sorted = playable.slice().sort((m1, m2) => score(m1) - score(m2));

  for (const [A, B] of sorted) {
    if (chosen.length >= courts) break;
    // no permitir que un equipo juegue dos veces en la misma ronda
    ...
    // actualiza PJ acumulados para el resto del calendario
...
function generateTeamRoundRobin(teams: any[], roundsLimit?: number) {
  ...
  // Usamos el algoritmo clásico: en cada ronda emparejamos arr[i] con arr[m-1-i]
  // y luego rotamos moviendo el último elemento a la posición 1.
  const arr = [...list];
  const rounds: any[][] = [];
  ...
  for (let r = 0; r < totalRounds; r++) {
    ...
    // Rotación: sacar último y ponerlo en la posición 1
    const last = arr.pop();
    if (last !== undefined) {
      arr.splice(1, 0, last);
    }
...
export default function App() {
  ...
  function generateSchedule() {
    ...
    if (mode === MODES.TEAMS) {
      ...
      // Mapa de partidos jugados por equipo (solo TEAMS)
      const gamesCount = new Map<string, number>(teams.map((t: any) => [t.id, 0]));

      // Convertimos extended (pairings por ronda) a schedule usando balanceo
    ...
    function randomizeNextRound() {
      ...
      // MODO INDIVIDUAL → se queda igual
      ...
      // MODO EQUIPOS FIJOS
      // Si todavía no tenemos plan, lo generamos
      if (!teamPlan || teamPlan.length === 0) {
        ...
        // Balancea usando los PJ que ya haya en el schedule (si hubiera)
      ...
      // Ya hay plan → usamos la siguiente ronda del plan y también balanceamos
